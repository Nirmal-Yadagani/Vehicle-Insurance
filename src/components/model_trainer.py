...
class ModelTrainer:
    ...
    def get_model_object_and_report(self, train: np.array, test: np.array) -> Tuple[object, object]:
        try:
            logging.info("Training RandomForestClassifier with the specified parameters")
            
            X_train, y_train = train[:,:-1], train[:,-1]
            X_test, y_test = test[:,:-1], test[:,-1]
            logging.info('features-target split done for both train and test data')
    ...
    def initiate_model_trainer(self) -> ModelTrainerArtifact:
        ...
        try:
            print("---"*50)
            print("Started model trainer component")
            ...
            logging.info('Saved final object that includes both preprocessing and trained model')
